...
(*tuples can have fun in it as well ?? We need to have a hetrogenous list*)


(* type operation = Add of integer*integer
...
(* exec stk_c stk_clos mycode [] [];; *)

(* app(app(lam(x, lam(y, x + y)), 2), 3) *)
(* let myprog2 = App( App( Abs("x", Abs( "y", Add(V "x",V "y") ) ) , C (Int 2) ), C (Int 3));;
...
(* 
let myprog3 = Elif( Equals( (C (Int 3)), (C (Int 3))) ,(C (Int 4)), (Div( (C (Int 8)),(C (Int 8)) )) );;
let mycode3 = compile myprog3;;
exec stk_c stk_clos mycode3 [] [];;
 *)


(* app(lam(f, app(f, 2), lam(y, y + 3)) *)
